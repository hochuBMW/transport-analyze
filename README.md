...
- `IRKBUS_SESSION_REFRESH_SEC` — плановое обновление `PHPSESSID` в секундах (по умолчанию `5400`, т.е. 90 мин; `0` — отключить).
...
Сборщик периодически «будит» сессию запросом на главную `irkbus.ru` (как F5 в браузере): cookie может не меняться, но сервер снова отдаёт данные в API. Делается по расписанию (~90 мин), после пустых ответов и при ошибках сети.

Если провайдер сразу возвращает пустые снимки (`buses=0`), можно передать стартовый cookie:
